"""Seed pooly + round-robin.

Jádro celé věci: LLM vybere 3–5 seed skladeb, z každé vytáhneme nezávislé
rádio, a při plnění fronty mezi pooly střídáme. Jeden seed sám ujede během
~20 skladeb k jednomu interpretovi; prokládání to udrží hodiny — a zadarmo,
protože LLM už do toho nevstupuje.
"""
...
class Pool:
    ...
    last_good: str = ""  # poslední nepřeskočená skladba — z ní přeseedujeme
...
class RadioPools:
    def __init__(self, catalog: Catalog, store: Store, cfg: Config) -> None:
        ...
        self._rr = 0  # round-robin ukazatel
        self.session_seen: set[str] = set()
        self.mood: str = ""
        # rejstřík všeho, co jsme v téhle session viděli — LLM pracuje jen
        # s videoId, tady si je překládáme zpátky na Track
    ...
    async def set_seeds(self, seeds: list[Track], mood: str = "") -> dict:
        """Nahradí pooly novými seedy a natáhne k nim rádia."""
        self.pools = []
        self._rr = 0
        self.mood = mood
        summary = []
    ...
    async def _fetch_radio(self, video_id: str) -> list[Track]:
        ...
        except Exception as exc:  # rádio může u některých ID selhat
    ...
    async def next_tracks(self, count: int) -> list[Track]:
        """Vytáhne `count` skladeb, střídavě z poolů, s aplikovanými filtry."""
        out: list[Track] = []
    ...
    def _acceptable(
        ...
        if track.duration is not None:
            if not (self.cfg.min_duration <= track.duration <= self.cfg.max_duration):
                return False
        # max 2 skladby od stejného interpreta v jednom doplnění
        if artist_counts.get(track.artist, 0) >= 2:
            return False
        return True

    async def _refill(self, pool: Pool) -> None:
        """Přeseeduje z poslední nepřeskočené skladby — implicitní zpětná vazba."""
    ...
    def mark_finished(self, video_id: str) -> None:
        """Skladba dohrála — dobrý signál, použij ji jako příští seed poolu."""
